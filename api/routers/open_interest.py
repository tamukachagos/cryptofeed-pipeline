...
router = APIRouter(tags=["open_interest"])
...
async def get_open_interest(
    ...
    engine = _get_engine()
    df = engine._load("open_interest", exchange.lower(), symbol.upper(), start_dt, end_dt)
    ...
    if format == "parquet":
        ...
        return StreamingResponse(buf, media_type="application/octet-stream",
                                 headers={"Content-Disposition": f"attachment; filename=oi_{exchange}_{symbol}_{start[:10]}_{end[:10]}.parquet",
    ...
    if format == "csv":
        ...
        return StreamingResponse(io.BytesIO(buf.getvalue().encode()), media_type="text/csv",
                                 headers={"Content-Disposition": f"attachment; filename=oi_{exchange}_{symbol}_{start[:10]}_{end[:10]}.csv",
                                          "X-Request-ID": str(uuid.uuid4())})
